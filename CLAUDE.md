...
El portal diario de producción de contenido de **Yo Desarrollo**. Cada mañana llega un correo con
la liga; el editor entra, palomea o tacha lo que se le propone, escribe su nota al margen y manda
su revisión. De esa revisión sale lo que la Mac produce ese día, y todo queda registrado día por
día.
...
2. **Nunca subir claves al repo.** Es público. Las 4 claves viven en `~/.sala_gas_claves.json` y la
   liga /exec en `~/.sala_gas` (ambos 600, verificado 4-sep); la copia pública del .gs trae `'…'`
   en lugar de la semilla (`gas/Code.gs:47`).
...
| `gas/appsscript.json` | Manifiesto. Lleva `webapp: {executeAs: USER_DEPLOYING, access: ANYONE_ANONYMOUS}` — sin eso, cada versión nueva nacía cerrada y el /exec daba 404 ([[sala-dos-editores]]). |
...
```
Mac (~/yod_audit)                Apps Script /exec              La Sala (GitHub Pages)
sala_publicar.py ─ láminas ────────────────────────────────────▶ repo (imágenes, tiras)
...
sala_relevo_diario.py ─ pendientes ─▶ PROPUESTAS (día vivo)
                      └─ respaldo espejo ─────────────────────▶ datos/manifiesto.json
                       ...
                       GET ?recurso=dia&clave=… ──────────────▶ index.html  (traer(), reintenta 3× / 12 s)
                       (si no contesta) ──────────────────────▶ datos/manifiesto.json  (index.html:534)

editor (Alejandro / Sayri) ─ accion:decidir ─▶ DECISIONES ─── el GAS toma el ÚLTIMO ENVÍO
                                                              DE CADA EDITOR y los fusiona
                                                              (vigentePorEditor, Code.gs:115).
                                                              **El «no» manda.**  v16.
...
- **⚠️ EL REPO ES ESPEJO.** Para subir código: `clasp push` → *Manage deployments* → lápiz →
  **New version** → Deploy (la URL no cambia). `clasp create-deployment`/`update-deployment`
  rompen la entrada web. Hay ~40 s de propagación con 404: verificar con varios curl antes de
  cantar victoria. Y para POST al GAS desde la Mac, **`curl -L`**: `urllib` dio 404 una vez.
...
Portero la renueve (commit `a026510`). Llaves en localStorage: `sala_gas`, `sala_clave`,
`sala_rol`, `sala_cola` (envíos pendientes), `sala2-<fecha>` (borrador del día).

## Decisiones

- **2026-08-01 · Alejandro + panel (Duolingo/Lingokids/Editor/Ingeniero)** — el ritual:
  UMBRAL → TURNO (una decisión por pantalla) → FIRMA → CIERRE → LA MESA. Porqué: tenía que caber
  en pocos taps; medido en 12 (límite 25).
- **2026-08-04 · conectada al Sheet** («Sala de Edición · YOD», Gmail personal). Porqué: ese día
  Alejandro revisó, su revisión quedó atrapada en el navegador y el día amaneció como si no
  hubiera decidido.
...
- **2026-08-29 · respaldo espejo** — la Sala servía tarjetas viejas porque caía al manifiesto
  congelado. Ahora el relevo lo reescribe en cada corrida. Porqué: los arreglos «no llegaban».
- **2026-09-02 · Alejandro (desde el iPhone)** — un **borrador más nuevo que el Sheet manda**: no
  ...
  pierden contra un envío del Sheet de las 08:04.
- **2026-09-03 · Alejandro** — *«los dos somos editores… contempla ambas y cumple los criterios
  de ambos»*: GAS **v16** fusiona el último envío de cada uno, **el «no» manda**, notas firmadas.
  La marca del OTRO editor **no canda** tu carta (llega con franja azul).
...
- **La copia del repo va atrás del script desplegado.** `gas/Code.gs` NO contiene las acciones
  ...
  [[sala-dos-editores]] describe como desplegadas el 3-sep. La cabecera del archivo aún dice
  «v3, espec de la mesa 1-ago» aunque el `version:` de la línea 230 ya diga
  `dos-editores-2026-09-03b`. **Pregunta: ¿bajamos la copia pública del editor para que el repo
  vuelva a ser espejo fiel?**
- **El id del Sheet «Sala de Edición · YOD»** no está en el repo (correcto: es privado). Vive en
  el Gmail personal según memoria; no verificable desde aquí.
- **La rama local `dominio-propio`** existe (`git branch`) y no tiene remoto. Pregunta: ¿sigue
  viva o se borra?
...
|---|---|---|
| DNS de `tableros.yodesarrollo.mx` | Alejandro (con Miguel Reina / cPanel) | `curl -I https://tableros.yodesarrollo.mx/sala-edicion/` → 200. Hoy da `000`. Al cortar hay que cambiar `PORTAL` en `gas/Code.gs:23` **a mano en el editor** y la liga de `sala_publicar.py:127`. |
...
| Métricas de La Servilleta en FB: alcance 0 (`datos/piezas.json`, 4-sep) | Alejandro | Explicación de por qué FB no repartió, o segunda medición. |
